...
=== "KSKB-HOME"
    ...
    * 無SLA保證
    ...
    === "VM Shape"
        ...
        ??? "IX 全域性規定"
            * 遵守中華民國（台灣）法律，禁止施行任何法律高風險行為，可能致使管理員電腦被扣押的舉動
            ...
            * 資源合理使用。禁止長時間消耗/佔滿CPU/網路頻寬等資源，例如rclone轉存/挖礦，**由管理員主觀判斷**
        ...
        ???+ PoP個別規定
            ...
            * 僅供個人使用，禁止轉讓/租借/商業使用
            * VM 提供的網路，僅供作為**成員間內網流量交換**使用，不得作為外部網路使用
                * 網路活動的連線對象必須符合以下資格
                    1. [自己內網的其他節點](Virtual-IXP.md#IX-VM-ETH0)
                    2. Peering 對象
        ...
        ???+ "VM 接入要求"
            * 你必須向 KSKB 證明你具有下列能力:
                1. 了解交換機轉發封包和路由器選路過程(L2轉發/L3路由的具體發生了什麼)
                2. 了解動態路由的目的，還有和靜態路由的差異
                3. 了解 eBGP/iBGP/iGP/RR 之間的差異，這些技術要解決的問題，還有運作的過程
                4. 必須要有自己的內網(可以基於隧道)，且必須把提供的 VM 加入內網
                5. 內網必需要有 iGP 和 iBGP 路由協議，能進行內網路由。並且能在內網區分 peer 和 transit 表
...
* 節點設備由加盟人所有及管理
* 管理員是加盟人
* 接入由該 PoP 之管理員同意，無須 KSKB 同意
* 但若發現有濫用行為，KSKB 有權要求管理員清退成員

### 贊助
* 設備由贊助人贊助，並非備由 KSKB 本人所有
* 管理員是 KSKB
* 接入需要 KSKB 本人同意
