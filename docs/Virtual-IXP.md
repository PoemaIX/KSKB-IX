...
* colo: 指 Colocation 。意思是租一個空間和電力，自己可以擺一台機器在機房內部
* 跳線: **同大樓機房內，不同機櫃之間的牽線服務**。機房內有很多機櫃。擺了很多機器。如果不同機櫃之間的機器要物理連線，就需要「跳線」。租用跳線服務以後，就可以在兩個機櫃之間牽一條光纖網路線，兩邊的機器就能連接起來
...
2. VM 接入: 
    ...
    2. 乘載網路: 模擬**外部進線**到自己的機器，讓大家打隧道去自己其他的內網節點。通常被稱作「上網卡(eth0)」
...
如果乘載網延遲很高，相當於所有進線部分的品質都不好，就會導致這在這邊交換的延遲都被拉高  
...
虛擬 IX 和物理 IX 類似，同樣也有「進線」的問題。但這部是**使用隧道來模擬**的  
...
虛擬 IX 用隧道**模擬進線**時。連線對象一定是「自己其他的內網節點」  
在 Poema IX 這邊，有規定提供的 IX VM 乘載網「**僅供模擬進線服務**，禁止用於其他目的」  
